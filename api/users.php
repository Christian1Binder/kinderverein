...
</style></head><body><main class="wrap"><p><a href="../">← Projektzentrale</a></p><div class="card"><small>Administration</small><h1>Benutzer verwalten</h1><p class="muted">Admin = Vollzugriff. Standardzugang = keine internen Sonderrechte; diese werden im Portal-Admin gezielt vergeben. Lesen = nur ansehen.</p><?php if($message):?><p class="ok"><?=h2($message)?></p><?php endif;?><?php if($error):?><p class="err"><?=h2($error)?></p><?php endif;?>
<form method="post" class="grid"><input type="hidden" name="csrf" value="<?=h2($csrf)?>"><input type="hidden" name="do" value="create"><label>Name<input required name="name"></label><label>E-Mail<input required type="email" name="email"></label><label>Passwort<input required minlength="12" type="password" name="password"></label><label>Rolle<select name="role"><option value="member">Standardzugang</option><option value="viewer">Lesen</option><option value="admin">Admin</option></select></label><button>Anlegen</button></form></div>
